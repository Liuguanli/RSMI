...
void ZM::point_query(ExpRecorder &exp_recorder, Point query_point)
{
    ...
    //     long mid = (front + back) / 2;
    //     LeafNode *leafnode = leafnodes[mid];
    //     // cout << "curve_val:" << curve_val << endl;
    //     // cout << "(*leafnode->children)[0].curve_val:" << (*leafnode->children)[0].curve_val << endl;
    //     // cout << "(*leafnode->children)[leafnode->children->size() - 1].curve_val:" << (*leafnode->children)[leafnode->children->size() - 1].curve_val << endl;
    //     exp_recorder.page_access++;
    //     if (curve_val < (*leafnode->children)[0].curve_val)
    //     {
    //         back = mid - 1;
    //     }
    //     else if (curve_val <= (*leafnode->children)[leafnode->children->size() - 1].curve_val)
    //     {
    //         int inner_front = 0;
    //         int inner_back = leafnode->children->size() - 1;
    //         // cout << "curve_val:" << curve_val << endl;

    //         while (inner_front <= inner_back)
    //         {
    //             long inner_mid = (inner_front + inner_back) / 2;
    //             Point point = (*leafnode->children)[inner_mid];
    //             // cout << "point.curve_val:" << point.curve_val << endl;
    //             // cout << "inner_front:" << inner_front << endl;
    //             // cout << "inner_back:" << inner_back << endl;

    //             if (point.curve_val > curve_val)
    //             {
    //                 inner_back = inner_mid - 1;
    //             }
    //             else if (point.curve_val < curve_val)
    //             {
    //                 inner_front = inner_mid + 1;
    //             }
    //             else
    //             {
    //                 if (point == query_point)
