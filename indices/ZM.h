...
void ZM::build(ExpRecorder &exp_recorder, vector<Point> points, int resolution, long long m)
{
    ...
    for (long long i = 0; i < N; i++)
    {
        ...
        points[i].xs[0] = points[i].x_i;
        points[i].y_i = points[i].y * N / resolution;
        points[i].xs[1] = points[i].y_i;
        long long curve_val = compute_Z_value(points[i].xs, 2, bit_num);
    ...
    for (size_t i = 0; i < stages.size(); i++)
    {
        ...
        for (size_t j = 0; j < stages[i]; j++)
        {
            ...
            try
            {
                ...
                // cout << "i: " << i << " j: " << j << " size: " << tmp_records[i][j].size() << endl;
                ...
                if (i == 0)
                {
                    ...
                    else if (exp_recorder.is_rs)
                    {
                        ...
                        // vector<Point> tmp_records_0_0 = pre_train_zm::get_representative_dataset(Constants::DATASETS, exp_recorder.get_file_name(), m, resolution);
                        ...
                        chrono::duration_cast<chrono::nanoseconds>(finish_rs - start_rs).count();
                        net->train_model(locations, labels);
                    }
                ...
                else
                {
                    ...
                    //         long long curve_val = compute_Z_value(tmp_records_i_j[i].x_i, tmp_records_i_j[i].y_i, bit_num);
