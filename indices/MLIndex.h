...
void MLIndex::build(ExpRecorder &exp_recorder, vector<Point> points)
{
    ...
    // TODO 1 prepare all data points
    // TODO 2 use kmeans to find k reference points
    auto start_z_cal = chrono::high_resolution_clock::now();
    get_reference_points(exp_recorder);

    // reference_points
    // TODO 3 for the partitions

    N = points.size();
    for (size_t i = 0; i < N; i++)
    {
        int partition_index = 0;
        partition_index = get_partition_id(points[i]);
        // float minimal_dist = numeric_limits<float>::max();
        // for (size_t j = 0; j < k; j++)
        // {
        //     float temp_dist = reference_points[j].cal_dist(points[i]);
        //     if (temp_dist < minimal_dist)
        //     {
        //         minimal_dist = temp_dist;
        //         partition_index = j;
    ...
    for (size_t i = 0; i < k; i++)
    {
        ...
        if (i != 0)
        {
            offsets[i] = offsets[i - 1] + maximal_dist;
        }
        else
        {
            offsets[i] = maximal_dist;
        }
        // cout << "partitions[i] size: " << partitions[i].size() << endl;
        // cout << "offsets[i]: " << i << " " << offsets[i] << endl;
    }
    // TODO 5 order the data
    for (size_t i = 0; i < N; i++)
    {
    ...
    max_key_val = points[points.size() - 1].key;
    for (size_t i = 0; i < N; i++)
    {
        // cout << "points[i]: " << points[i].key << endl;
        points[i].index = i * 1.0 / N;
        points[i].normalized_curve_val = (points[i].key - min_key_val) * 1.0 / gap;
    }
    // TODO 6 use ZM index structure to train the data
    // ---------------------------ZM---------------------------
...
void MLIndex::point_query(ExpRecorder &exp_recorder, Point query_point)
{
    ...
    query_point.normalized_curve_val = (query_point.key - min_key_val) * 1.0 / gap;
