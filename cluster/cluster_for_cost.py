...
def parser(argv):
    try:
        opts, args = getopt.getopt(argv, "d:k:f:")
    ...
    for opt, arg in opts:
        if opt == '-d':
            ...
            filename = arg
    return dataset, k, filename

if __name__ == "__main__":
    dataset, k, filename = parser(sys.argv[1:])
    X = load_data("/home/liuguanli/Documents/pre_train/2D_data/%s.csv" % (dataset))
    # test_mini_batch_kmeans_manual(X)
    test_mini_batch_kmeans_auto(X, k, filename % (dataset))
    # for i in range(4):
    # 23854
    # 99457101
