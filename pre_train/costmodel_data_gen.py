...
def gen_uniform_points(num, filename, dim):
    locations = []
    for i in range(num):
        temp = []
        for j in range(dim):
...
def gen_normal_points(num, filename, dim, mean=0.5, stddev=0.125):
    locations_tf = []
    for i in range(dim):
...
def gen_skewed_points(num, a, filename, dim):
...
def gen(distribution, size, dim):
    if distribution == 'uniform':
        ...
        gen_uniform_points(size, filename, dim)
        ...
        for mean in means:
            for stddev in stddevs:
                gen_normal_points(size, filename, dim, mean, stddev)
        ...
        for skewness in  skewnesses:
            gen_skewed_points(size, skewness, filename, dim)
